...
import NovaObraPageClient from "./NovaObraPage-client"
...
export default async function NovaObraPage() {
  ...
  if ("error" in funcionariosListResult) {
    return (
      <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
        <h2 className="text-2xl md:text-3xl font-bold tracking-tight">Obras</h2>
        <div className="flex items-center justify-center h-96">
          <p className="text-red-500 text-lg">{funcionariosListResult.error}</p>
        </div>
      </div>
    )
  }
  ...
  return <NovaObraPageClient funcionariosDisponiveis={funcionariosListResult} />
}
